...
>
> **说明**：本仓库是 [JDCK380/Funnyass_school](https://github.com/JDCK380/Funnyass_school) 的 fork。
> 下面的记录**只覆盖 fork 之后的改动**（`0.1920` 起）。上游作者的原始工作不在本文档范围内。
> 上游的协议解析、蓝牙收发、登录与余额接口逻辑基本沿用，本仓库只做必要修改。
...
### 1.0.7 — 发布版本，用于验证应用内更新（当前版本）

功能与 1.0.6 相同，仅作版本递增，用于验证 1.0.6 新做的**应用内下载安装**是否能走通。

**顺带**：去掉按压反馈里每次触摸都记录的 `press down` 日志（噪音太大），
只保留齿轮旋转的 `spin 开始 / spin 结束`，便于排查旋转问题。
...
这条信息把范围缩小到一点：触摸事件到达了控件、缩放动画也确实执行了，
问题只可能出在「回弹结束后触发旋转」这一环。核对后确认有两个原因叠加：

**原因 1：旋转挂在不可靠的回调上**

原实现把旋转放在 `ViewPropertyAnimator.withEndAction` +
`view.isPressed` 里。`withEndAction` 在触摸手势期间并不可靠（会被后续动画取消），
而缩放走的是同一条 `animate()` 路径却正常，正好印证了问题在回调而非动画本身。

→ 改为**在 `setOnClickListener` 里触发旋转**。点击回调本身就只在有效点击时执行，
天然带边界过滤，不需要 `isPressed` 这个中间状态。
`attach()` 也就只管缩放，职责更单一。

**原因 2：动画被弹窗挡住了**

设置弹窗从**底部**滑入，而齿轮在**右上角**——旋转即使执行了，也会连同主界面一起被盖住。
所以又加了一条：点击后先让齿轮转，**延迟 260ms 再打开弹窗**（`SPIN_LEAD_MS`），
确保用户能看到旋转。

**顺带**：旋转动画改用 `ViewPropertyAnimator.rotationBy(360f)`，
与缩放同一套机制，也不再需要缓存角度（`rotationBy` 天然基于当前角度累加）。

> 状态：修复已完成并构建通过，**手感待真机确认**。
...
**1. 按压反馈**（`PressFeedback.attach`）

按下缩小到 96%，松开用 `OvershootInterpolator` 弹回。已接入：
设置齿轮、蓝牙设备卡片、开始洗澡、停止并结算、连接此设备、重新扫描、断开连接、重新登录。

两个细节：

- touch 监听返回 `false`，**不消费事件**，点击/长按等原有逻辑照常工作
- 灰掉的按钮（`isEnabled == false`）不做按压动画

**2. 齿轮旋转**（`PressFeedback.spinOnce`）

点设置按钮时齿轮转一圈（520ms，`DecelerateInterpolator`）。
每次从上次角度继续转（缓存角度），不会每次从 0 重来；旋转中忽略连点，不会叠加。
转的是 `settings_btn` 本身——它的背景是纯圆，旋转后视觉不变，看起来就是齿轮在转。

触发时机是**松手且 `isPressed` 仍为 true**，所以手指滑出按钮再松开不会误触发。

**3. 弹窗进出场**（`SheetAnim`）

- 设备列表 / 设置：从下方 48dp 滑入 + 淡入（带一点回弹）；缩回时下滑 + 淡出，动画结束才 `GONE`
- 遮罩：淡入淡出
- 「A 弹窗换成 B」用立即隐藏（`hideSheets(animate = false)`），
  否则旧的缩回动画会和新的弹出动画叠在一起
- 进入全屏子页面（使用记录 / 诊断日志）时**保留遮罩**，避免返回设置页时遮罩重新淡入闪一下

> 注：本版本只做了构建与静态校验。模拟器是 x86_64，而工程只带 `arm64-v8a` 原生库，
> 跑不起来，因此动画手感需要真机确认。

---

### 1.0.2 — 登录页 Logo 与软件图标统一
